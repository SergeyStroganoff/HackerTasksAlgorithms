    ...
    public List<Integer> getLowestSubSequenceNLLogN(int[] array) {
      ...
      //  indexes[0] = 0; //index of first element of subsequence
        ...
        for (int i = 1; i < array.length; ++i) { //start from second element
            indexes[i] = ceilIndex(memoryArray, 0, indexOfLongestSubsequence + 1, array[i]); //find ceil index for current element in subsequence array and save it in indexes array for current element of array sequence
