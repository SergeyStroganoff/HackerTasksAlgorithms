 ...
 * <p>
 * Если задача стоит только найти только расстояние до конкретной ноды, то как только находим то выходим сразу из цикла
 * break;
 */
...
public class DijkstraAlgo {
    public static Map<Integer, Integer> dijkstra(Map<Integer, List<int[]>> graph, int start) {
        ...
        distMap.put(start, 0);  // переназначаем стартовой ноде расстояние до нее 0
        ...
        while (!priorityQueue.isEmpty()) { // поиск в ширину пока очередь не пуста
            ...
            //ищем в мапе ноду и если есть у нее расстояние то сравниваем
            ...
            /**
             * в цикле перебираем всех родственников берем у них номер ноды и расстояние до неё
             * вычисляем дистанцию до ноды путем прибавления расстояние текущей ноды + расстояние до родственника
             * проверяем в мапе расстояний - если полученное расстоянее меньше уже имеющего (оптимальнее) то
